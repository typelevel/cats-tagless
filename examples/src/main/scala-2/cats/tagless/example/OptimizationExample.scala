...
object OptimizationExample extends App {
  ...
  val programInstance = new Program[KVStore, Applicative, List[String]] {
    def apply[F[_]: Applicative](alg: KVStore[F]): F[List[String]] =
      program(alg)
  }
  ...
  def createOptimizer[F[_]: Monad]: Optimizer[KVStore, F] =
    new Optimizer[KVStore, F] {
      ...
      def rebuild(keys: Set[String], interp: KVStore[F]): F[KVStore[F]] = {
        ...
        keys.toList
          .traverse(key => interp.get(key).map(_.map(value => key -> value)))
          .map { results =>
